...
def test_max_frame_time():
    ...
    assert instance.host_tshark_max_frame_time_in([
        {'eth.src': TEST_MAC, 'frame.time_epoch': 999},
        {'eth.src': TEST_MAC, 'frame.time_epoch': 1001}]) == [
                {'host_key': TEST_MAC, 'max_frame_time_in': 1001}]


def test_max_frame_len():
    instance = Host()
    assert instance.host_tshark_max_frame_len([{'eth.src': TEST_MAC, 'frame.len': 999}]) == [
        {'host_key': TEST_MAC, 'max_frame_len': 999}]


def test_ipversions():
    instance = Host()
    assert instance.host_tshark_ipv4([{'eth.src': TEST_MAC, 'ip.version': 4}]) == [
        {'host_key': TEST_MAC, 'IPv4': 1}]
    assert instance.host_tshark_ipv6([{'eth.src': TEST_MAC, 'ip.version': 6}]) == [
        {'host_key': TEST_MAC, 'IPv6': 1}]


def test_protocols():
    instance = Host()
    assert instance.host_tshark_last_protocols_array([{'eth.src': TEST_MAC, 'frame.protocols': 'eth:ethertype:ip:udp:db-lsp-disc:json'}]) == [
    ...
    assert instance.host_tshark_last_protocols_array([{'eth.src': TEST_MAC}]) == [
...
def test_non_ip():
    instance = Host()
    assert instance.host_tshark_non_ip([{'eth.src': TEST_MAC, 'eth.type': 99}]) == [
        {'host_key': TEST_MAC, 'tshark_non_ip': 1}]
    assert instance.host_tshark_non_ip([{'eth.src': TEST_MAC, 'eth.type': 0x00000800}]) == [
        {'host_key': TEST_MAC, 'tshark_non_ip': 0}]


def test_vlan_id():
    instance = Host()
    assert instance.host_tshark_vlan_id([{'eth.src': TEST_MAC, 'vlan.id': 999}]) == [
        {'host_key': TEST_MAC, 'tshark_vlan_id': 999}]


def test_ipx():
    instance = Host()
    assert instance.host_tshark_ipx([{'eth.src': TEST_MAC, 'eth.type': 0x8137}]) == [
        {'host_key': TEST_MAC, 'tshark_ipx': 1}]
    assert instance.host_tshark_ipx([{'eth.src': TEST_MAC, 'eth.type': 0x800}]) == [
        {'host_key': TEST_MAC, 'tshark_ipx': 0}]


def test_both_private_ip():
    instance = Host()
    assert instance.host_tshark_both_private_ip([{'eth.src': TEST_MAC, 'ip.src': '192.168.0.1', 'ip.dst': '10.10.1.1'}]) == [
        {'host_key': TEST_MAC, 'tshark_both_private_ip': 1}]
    assert instance.host_tshark_both_private_ip([{'eth.src': TEST_MAC, 'ip.src': '192.168.0.1', 'ip.dst': '1.1.1.1'}]) == [
        {'host_key': TEST_MAC, 'tshark_both_private_ip': 0}]


def test_ipv4_multicast():
    instance = Host()
    assert instance.host_tshark_ipv4_multicast([{'eth.src': TEST_MAC, 'ip.src': '192.168.0.1', 'ip.dst': '224.0.0.1'}]) == [
        {'host_key': TEST_MAC, 'tshark_ipv4_multicast': 1}]
    assert instance.host_tshark_ipv4_multicast([{'eth.src': TEST_MAC, 'ip.src': '192.168.0.1', 'ip.dst': '10.0.0.2'}]) == [
        {'host_key': TEST_MAC, 'tshark_ipv4_multicast': 0}]


def test_tcp_flags():
    instance = Host()
    rows = [{'eth.src': TEST_MAC, 'tcp.flags': 0x00000014}]
...
def test_ip_flags():
    instance = Host()
    rows = [{'eth.src': TEST_MAC, 'ip.flags': 0x00004000}]
...
def test_ip_dsfield_flags():
    instance = Host()
    rows = [{'eth.src': TEST_MAC, 'ip.dsfield': 0x000000c0}]
...
def test_tcp_priv_ports():
    instance = Host()
    rows = [{'eth.src': TEST_MAC, 'tcp.srcport': 1025, 'tcp.dstport': 80}]
...
def test_udp_priv_ports():
    instance = Host()
    rows = [{'eth.src': TEST_MAC, 'udp.srcport': 1025, 'udp.dstport': 123}]
...
def test_tcp_nonpriv_ports():
    instance = Host()
    rows = [{'eth.src': TEST_MAC, 'tcp.srcport': 1025, 'tcp.dstport': 9999}]
...
def test_wk_ip_protos():
    instance = Host()
    assert instance.host_tshark_wk_ip_protos([{'eth.src': TEST_MAC, 'tcp.something': 'whatever', 'udp.something': 'whatever', 'something.else': 'something', 'data.something': 'whatever'}]) == [
