...
def test_get_individual_predictions():
   ...
   instance = HostFootprint()
   assert instance.get_individual_predictions([[0.6, 0.7]], le, filename, host_key, tshark_srcips) == {
        'firstfile': {'top_role': 'bsomething', 'role_list': [('bsomething', 0.7), ('asomething', 0.6)], 'source_ip': '1.1.1.1', 'source_mac': 'mac1'}}
   assert instance.get_individual_predictions([[0.2, 0.1]], le, filename, host_key, tshark_srcips) == {
