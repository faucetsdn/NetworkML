...
eval_onelayer_nobuild:
	...
	@docker run -it --rm -v "$(PCAP):/pcap/$(PCAP)" --link poseidonml-redis:redis -e RABBIT=false -e POSEIDON_PUBLIC_SESSIONS=1 -e LOG_LEVEL=$(LOG_LEVEL) --entrypoint=python3 poseidonml:onelayer eval_OneLayer.py /pcap/$(PCAP)
...
test_onelayer_nobuild:
	...
	@docker run -it --rm -v "/tmp/models:/OneLayer/models" -v "$(PCAP):/pcaps/" --link poseidonml-redis:redis -e RABBIT=false -e LOG_LEVEL=$(LOG_LEVEL) --entrypoint=python3 poseidonml:onelayer test_OneLayer.py
	@docker rm -f poseidonml-redis > /dev/null
	@echo
...
train_onelayer_nobuild:
	...
	@docker run -it --rm -v "/tmp/models:/OneLayer/models" -v "$(PCAP):/pcaps/" --link poseidonml-redis:redis -e RABBIT=false -e LOG_LEVEL=$(LOG_LEVEL) --entrypoint=python3 poseidonml:onelayer train_OneLayer.py
	@docker rm -f poseidonml-redis > /dev/null
	@echo
...
eval_randomforest_nobuild:
	...
	@docker run -it --rm -v "$(PCAP):/pcaps/$(PCAP)" --link poseidonml-redis:redis -e RABBIT=false -e LOG_LEVEL=$(LOG_LEVEL) --entrypoint=python3 poseidonml:randomforest eval_RandomForest.py /pcaps/$(PCAP)
	@docker rm -f poseidonml-redis > /dev/null
	@echo
...
test_randomforest_nobuild:
	...
	@docker run -it --rm -v "/tmp/models:/RandomForest/models" -v "$(PCAP):/pcaps/" --link poseidonml-redis:redis -e RABBIT=false -e LOG_LEVEL=$(LOG_LEVEL) --entrypoint=python3 poseidonml:randomforest test_RandomForest.py
	@docker rm -f poseidonml-redis > /dev/null
	@echo
...
train_randomforest_nobuild:
	...
	@docker run -it --rm -v "/tmp/models:/RandomForest/models" -v "$(PCAP):/pcaps/" --link poseidonml-redis:redis -e RABBIT=false -e LOG_LEVEL=$(LOG_LEVEL) --entrypoint=python3 poseidonml:randomforest train_RandomForest.py
	@docker rm -f poseidonml-redis > /dev/null
	@echo
...
eval_sosmodel_nobuild:
	...
	@docker run -it --rm -v "$(PCAP):/pcaps/$(PCAP)" --link poseidonml-redis:redis -e RABBIT=false -e LOG_LEVEL=$(LOG_LEVEL) --entrypoint=python3 poseidonml:sosmodel eval_SoSModel.py /pcaps/$(PCAP)
	@docker rm -f poseidonml-redis > /dev/null
	@echo
...
train_sosmodel_nobuild:
	...
	@docker run -it --rm -v "/tmp/models:/new_models" --link poseidonml-redis:redis -v "$(PCAP):/pcaps/" -e RABBIT=false -e LOG_LEVEL=$(LOG_LEVEL) --entrypoint=python3 poseidonml:sosmodel train_SoSModel.py /pcaps/ /models/SoSModel.pkl
	@docker rm -f poseidonml-redis > /dev/null
	@echo
run_redis:
