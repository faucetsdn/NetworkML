...
eval_naivebayes: build_naivebayes
	...
	@docker run -it -v "$(PCAP):/pcaps/eval.pcap" -e SKIP_RABBIT=true --entrypoint=python3 poseidonml:naivebayes eval_NaiveBayes.py
test_naivebayes: build_naivebayes
	@echo "Running NaiveBayes Test on PCAP files $(PCAP)"
	@docker run -it -v "/tmp/models:/NaiveBayes/models" -v "$(PCAP):/pcaps/" -e SKIP_RABBIT=true --entrypoint=python3 poseidonml:naivebayes test_NaiveBayes.py
train_naivebayes: build_naivebayes
	@echo "Running NaiveBayes Train on PCAP files $(PCAP)"
	@docker run -it -v "/tmp/models:/NaiveBayes/models" -v "$(PCAP):/pcaps/" -e SKIP_RABBIT=true --entrypoint=python3 poseidonml:naivebayes train_NaiveBayes.py
