...
class CSVToFeatures():
    ...
    def exec_features(self, features, in_file, out_file, features_path):
        rows = CSVToFeatures.get_rows(in_file)
        ...
        if header and rows:
            CSVToFeatures.write_features_to_csv(header, rows, out_file)
        else:
            self.logger.warning(f'No results based on {features} for {in_file}')

    def process_files(self, threads, features, features_path, in_paths, out_paths):
        num_files = len(in_paths)
        finished_files = 0
        # corner case so it works in jupyterlab
        if threads < 2:
            for i in range(len(in_paths)):
                self.exec_features(features, in_paths[i], out_paths[i], features_path)
        else:
            with concurrent.futures.ProcessPoolExecutor(max_workers=threads) as executor:
                future_to_parse = {executor.submit(self.exec_features, features, in_paths[i], out_paths[i], features_path): i for i in range(len((in_paths)))}
    ...
    def main(self):
        ...
        self.process_files(threads, features, features_path, in_paths, out_paths)
